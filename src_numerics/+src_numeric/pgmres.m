...
function [x,flag,relres,iter,resvec] = pgmres(A,b,restart,tol,maxit,L1,pL_L,L2,R1,R2,x0)
...
if L2pre
    if strcmp(L2type,'matrix')
        ...
        bp = src_numeric.iterapp('mtimes',L2fun,L2type,L2fcnstr,bp);
    end
end
...
while(resvec(it) > tol) && (outit < maxit)
    ...
    % increase external loop counter3
    outit = outit+1;
    
   % Call the gmres to perform restart iterations
   [x,r,p,resvec_inner] = src_numeric.pgmres_iter(A,x,r,restart,L1,pL_L,L2,R1,R2,tol*bnorm);
