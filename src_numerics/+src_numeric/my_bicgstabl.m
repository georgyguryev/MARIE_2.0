function [x,flag,relres,iter,resvec] = my_bicgstabl(A,b,tol,maxit,M1,p1,M2,x0,varargin)
...
for kk = 1 : maxit
    ...
    for jj = 1 : ell
        ...
        if existM1
            put = my_iterapp('mldivide',m1fun,p1,m1type,m1fcnstr,ut(:,jj),varargin{:});
            if ~all(isfinite(put))
                flag = 2;
                resvec = resvec(1:2*ell*(kk-1)+jj);
                break
        ...
        if existM1
            prt = my_iterapp('mldivide',m1fun,p1,m1type,m1fcnstr,rt(:,jj),varargin{:});
            if ~all(isfinite(prt))
                flag = 2;
                resvec = resvec(1:2*ell*(kk-1)+jj);
                break
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
            ...
            if existM1
                pxt = my_iterapp('mldivide',m1fun,p1,m1type,m1fcnstr,xt,varargin{:});
                if ~all(isfinite(pxt))
                    flag = 2;
...
if flag == 0
    ...
    if existM1
        pxt = my_iterapp('mldivide',m1fun,p1,m1type,m1fcnstr,xmin,varargin{:});
        pxt2 = my_iterapp('mldivide',m1fun,p1,m1type,m1fcnstr,xt,varargin{:});
