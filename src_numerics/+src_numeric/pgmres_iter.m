...
function [x,r,k,resvec] = pgmres_iter(A,x,r,m,L1,pL_L,L2,R1,R2,tol)
...
if(existR1) % first right preconditioning
    if strcmp(R1type,'matrix')
        ...
        xupd = src_numeric.iterapp('mtimes',R1fun,R1type,R1fcnstr,xupd);
    end
end
if(existR2) % second right preconditioning
    if strcmp(R2type,'matrix')
        ...
        xupd = src_numeric.iterapp('mtimes',R2fun,R2type,R2fcnstr, xupd);
    end
end   
