function [x,flag,relres,iter,resvec] = my_tfqmr(A,b,tol,maxit,M1,M2,pL,x0,varargin)
...
if (n2b == 0)                      % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        src_numeric.itermsg('tfqmr',tol,maxit,0,flag,iter,NaN);
    end
    return
end
...
if (normr <= tolb)                 % Initial guess is a good enough solution
    ...
    resvec = normr;
    if (nargout < 2)
        src_numeric.itermsg('tfqmr',tol,maxit,0,flag,iter,relres);
    end
    return
end
...
v = src_numeric.iterapp('mtimes',afun,atype,afcnstr,pu_m,varargin{:});
...
%         pu_m = src_numeric.iterapp('mtimes',m2fun,m2type,m2fcnstr,pu_m,varargin{:});
    ...
    Au_new = src_numeric.iterapp('mtimes',afun,atype,afcnstr,pu_m,varargin{:});
...
if flag == 0
    ...
    r_comp = b - src_numeric.iterapp('mtimes',afun,atype,afcnstr,xmin,varargin{:});
    if flag == 1 || flag == 3
        r_comp2 = b - src_numeric.iterapp('mtimes',afun,atype,afcnstr,x,varargin{:});
...
if nargout < 2
    src_numeric.itermsg('tfqmr',tol,maxit,mm,flag,iter,relres);
end
