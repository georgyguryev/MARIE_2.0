function [x,flag,relres,iter,resvec] = my_bicgstab(A,b,tol,maxit,M1,pL,M2,x0,varargin)
...
r = b - src_numeric.iterapp('mtimes',afun,atype,afcnstr,x,varargin{:});
...
for ii = 1 : maxit
    ...
    if existM1
        ...
        if strcmp(m1type,'matrix')
            ...
            ph = src_numeric.iterapp('mldivide',m1fun,m1type,m1fcnstr,p,varargin{:});
        end
        if ~all(isfinite(ph))
            flag = 2;
            resvec = resvec(1:2*ii-1);
    ...
    if existM2
        ph = src_numeric.iterapp('mldivide',m2fun,m2type,m2fcnstr,ph,varargin{:});
        if ~all(isfinite(ph))
            flag = 2;
            resvec = resvec(1:2*ii-1);
            break
    ...
    v = src_numeric.iterapp('mtimes',afun,atype,afcnstr,ph,varargin{:});
    rtv = rt' * v;
    if (rtv == 0) || isinf(rtv)
        flag = 4;
        resvec = resvec(1:2*ii-1);
    ...
    if (normr <= tolb || stag >= maxstagsteps || moresteps)
        s = b - src_numeric.iterapp('mtimes',afun,atype,afcnstr,xhalf,varargin{:});
        normr_act = norm(s);
    ...
    if existM1
        ...
        if strcmp(m1type,'matrix')
            ...
            sh = src_numeric.iterapp('mldivide',m1fun,m1type,m1fcnstr,s,varargin{:});
        end
        if ~all(isfinite(sh))
            flag = 2;
            resvec = resvec(1:2*ii);
    ...
    if existM2
        sh = src_numeric.iterapp('mldivide',m2fun,m2type,m2fcnstr,sh,varargin{:});
        if ~all(isfinite(sh))
            flag = 2;
            resvec = resvec(1:2*ii);
            break
    ...
    t = src_numeric.iterapp('mtimes',afun,atype,afcnstr,sh,varargin{:});
    
    %% compute omega using symmetric prec if applicable
    
    if existM1
        Ks = src_numeric.iterapp('mldivide', m1fun,m1type,m1fcnstr, s, varargin{:});
%         Ks = my_iterapp('mldivide', m2fun,p1, m2type,m2fcnstr, Ks, varargin{:});
        Kt = src_numeric.iterapp('mldivide', m1fun, m1type,m1fcnstr, t, varargin{:});
%         Kt = my_iterapp('mldivide', m2fun,p1, m2type,m2fcnstr, Kt, varargin{:});
        tt = Kt' * Kt;
        omega = (Kt'* Ks) / tt;
    else
        tt = t' * t;
...
if flag == 0
    ...
    r = b - src_numeric.iterapp('mtimes',afun,atype,afcnstr,xmin,varargin{:});
    if norm(r) <= normr_act
        x = xmin;
        iter = imin;
        relres = norm(r) / n2b;
