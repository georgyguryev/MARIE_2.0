...
function [field xs ys zs]=read_fld_field(filepath)
...
while Nleft > 0
    ...
    if Nleft >= Bsize  % if enough entries are left, read an entire block
        BLOCK = reshape( fscanf( file , '%f' , Bsize*9 ) , 9 , Bsize )';
        nBLOCK = Bsize;
        Nleft = Nleft - nBLOCK;
        
    else  % otherwise, read whatever is left in the file
        
        BLOCK = reshape( fscanf( file , '%f' , Nleft*9 ) , 9 , Nleft )';
        nBLOCK = Nleft;
        Nleft = Nleft - nBLOCK;
    end
    ...
    for ii = 1 : nBLOCK
        ...
        field(i,j,k,1)=tmp(4) + 1j*tmp(5);
        field(i,j,k,2)=tmp(6) + 1j*tmp(7);
        field(i,j,k,3)=tmp(8) + 1j*tmp(9);
