function [Z,F,feed_tune_ports] = Assembly_SIE_par(SCOIL,freq) 
...
% -------------------------------------------------------------------------
%            Update impedance matrix with lumped element models  
% -------------------------------------------------------------------------

tic

for pnum = 1:length(fixed_load_ports)
    
    % get current load port
    port_num = fixed_load_ports(pnum);

    % assembly excitation 
    [Z] = lumped_elem_assembly(Z,index,node,edge,port(port_num),freq);
end
