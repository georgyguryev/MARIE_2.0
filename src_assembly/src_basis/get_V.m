function [V_out] = get_V(rows, task_settings, scatterer, coil, dims, freq, op_type)
%  function get_V() assembles rows of coupling operator; 
...
switch op_type
    case 'N'                                                
        [V_N, ~] = src_coupling.assemble_coupling_matrices(task_settings, scatterer, coil, dims, freq, [], I_vox, 0);
        V_N = P_mask * V_N;
    case 'K'
        [~, V_K] = src_coupling.assemble_coupling_matrices(task_settings, scatterer, coil, dims, freq, [], I_vox, 0);
        V_K = P_mask * V_K;
    otherwise
        [V_N, V_K] = src_coupling.assemble_coupling_matrices(task_settings, scatterer, coil,  dims, freq, [], I_vox, 0);
        V_N = P_mask * V_N;
        V_K = P_mask * V_K;
end
