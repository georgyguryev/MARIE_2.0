function [adjacency_type,ordered_points] = points_mapping(m,dx)

    %% inputs and outputs 

    % m              : indices of the observation voxel indicating the voxel-voxel interaction
    % dx             : resolution
    
    % adjacency_type : (6x6) integer array with the singularity type 
    % orderd_points  : (3x7x6x6) double array with the 7 3D points for every m and adjacency type. For ST the last 3 points are zero, for EA the last 1.

    
    %% calculate the 3D points for all the adjacency types for all the nearby voxel-voxel interactions
    ...
    a = -dx/2;
    b = +dx/2;
    % source points - held stable - source voxel withe centre r_n = (0,0,0)
    p1 = [a a a]';
    p2 = [b a a]';
    p3 = [b b a]';    % observation points - moved - observation voxel with centre r_m = dr
